...
type Inst = { type: 'mul'; n1: number; n2: number } | { type: 'do' } | { type: "don't" };
type Input2 = Array<Inst>;

export function parseInput1(data: string): Input1 {
  const inputRegex = /mul\((\d{1,3}),(\d{1,3})\)/g;

  return [...data.matchAll(inputRegex)].map(([_, n1, n2]) => [Number(n1), Number(n2)]);
}
...
export function parseInput2(data: string): Input2 {
  ...
  return [...data.matchAll(inputRegex)].map(([_, typeMul, n1, n2, typeDo, typeDont]) => {
