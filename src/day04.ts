...
function matchWord(input: string, searchIdx: number, stride: number): boolean {
  return (
    ...
    input[searchIdx + stride * 1] == 'M' &&
    input[searchIdx + stride * 2] == 'A' &&
    input[searchIdx + stride * 3] == 'S'
  );
}
...
export function part1([input, rowLen]: [string, number]): number {
  ...
  return forEachChar(
    ...
    (searchIdx) =>
      directions
        .map((direction) => matchWord(input, searchIdx, direction))
        .filter((v) => v).length,
  );
}
...
export function part2([input, rowLen]: [string, number]): number {
  return forEachChar(input, 'A', (searchIdx) =>
    matchShape(input, searchIdx, rowLen) ? 1 : 0,
  );
}
